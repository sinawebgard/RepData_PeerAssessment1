...
*Note: the above information was last evaluated at `r Sys.time()` while this document has been processed.*
...
As we can see the range of mean for steps per intervals is much wider than the median. Also it seems for most of intervals the median number of steps across all days is zero. In total the median number of steps for `r sort(table(alt_value$steps), decreasing = TRUE)[1]` intervals is zero.
...
In total the number of steps in `r sort(table(dat$steps), decreasing = TRUE)[1]` intervals is equal to zero. That is `r sort(table(dat$steps), decreasing = TRUE)[1]/sum(complete.cases(dat))*100`% of observations excluding missing data.
